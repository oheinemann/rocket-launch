...
if (!(Get-Command powershell)) {
    Write-Output "PowerShell is not installed. Installing now..."
    # Install PowerShell
    Invoke-WebRequest -UseBasicParsing -Uri "https://aka.ms/install-powershell.ps1" -OutFile "$env:temp\install-powershell.ps1";
    & "$env:temp\install-powershell.ps1" -Quiet -NoRestart
}
...
if (!(Get-WindowsCapability -Name OpenSSH.Server* -Online)) {
    Write-Output "OpenSSH Server is not installed. Installing now..."
    Add-WindowsCapability -Name OpenSSH.Server* -Online
    # Start the SSH service
    Start-Service ssh-agent
    Start-Service ssh-service

    # Configure ssh-agent to start automatically
    Set-Service -Name ssh-agent -StartupType Automatic
    # Configure ssh-service to start automatically
    Set-Service -Name ssh-service -StartupType Automatic
}
...
if (!(Get-Service -Name "winrm" -ErrorAction SilentlyContinue)) {
    Write-Output "WinRM is not enabled. Enabling now..."
    winrm quickconfig
}
    ...
    if ($distro.AppxName.Length -gt 1) {
        ...
        if ($distro.sideloadreqd){
            if (Check-Sideload){
                ...
                wsl.exe -d $Distro.Name -u root useradd --shell /bin/bash --badnames --create-home --user-group --groups adm,dialout,cdrom,floppy,sudo,audio,dip,video,plugdev,netdev --password "$plainPwd" $user
            ...
            Write-Host($user + ": " + $plainPwd)
            Write-Output "wsl.exe -d $Distro.Name -u root useradd --shell /bin/bash --badnames --create-home --user-group --groups adm,dialout,cdrom,floppy,sudo,audio,dip,video,plugdev,netdev --password \"$plainPwd\" $user"
